...
public class Macro3DImpl implements Macro3D{
	...
	public Macro3DImpl(String projectName, boolean writeOutput,
			MacroInput input, RoadSection roadSection) {
		...
		if (writeOutput) {
			...
			writer.printf(Constants.COMMENT_CHAR
					+ "%-12s  %-12s  %-12s  %-12s  %-12s%n", "x[km]", "t[h]",
					"rho[1/km]", "V[km/h]", "Q[1/h]");
			writer.flush();
		}
	...
	private void calcData(double time, VehicleContainer vehContainer) {
		...
		double[] vMicro = new double[size];
		double[] xMicro = new double[size];
	...
	private void writeOutput(double time) {
		for (int j = 0; j < rhoInvKm.length; j++) {
			final double x = j * dxOut;
			writer.printf("%.6f  %.6f  %f  %f  %f %n", x / 1000., time / 3600.,
					rhoInvKm[j], vKmh[j], qInvH[j]);
		}
