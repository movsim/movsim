...
*/
    ...
    // the dimensionless epsilon has similar effects as the braking probability of the Nagel-S CA
...
* the v0 local
* @param TLocal
* the local time gap. Notice that inconsistencies may arise for nontrivial values
* since then no longer dt=T=tau_relax making the vSafe formula possibly inconsistent
* @return the double
*/
    private double acc(double s, double v, double dv, double v0Local, double TLocal) {
        final double vp = v - dv;
 
       /* safe speed; I checked that the complicated formula in PRE 55, 5601 (1997)
    is essentially my vSafe formula for the simple Gipps model. 
        The complicated formula considers effects of finite dt; this is treated uniformly 
        for all models in our update routine, so it is not necessary here.
        Therefore, I chose the simple Gipps vSafe formula
    see cmp_vsafe_GippsKrauss.gnu for details
    */
        final double vSafe = -b * T + Math.sqrt(b * b * T * T + vp * vp + 2 * b * Math.max(s - s0, 0.));

    /* vUpper =upper limit of new speed (denoted v1 in PRE)
       corresponds to vNew of the Gipps model */
    ...
    //===============================================
        ...
        /* vLower =lower limit of new speed (denoted v0 in PRE)
       some modifications due to dimensional units were applied.
       Notice that vLower may be > vUpper in some cut-in situations:
           these inconsistencies
       were not recognized/treated in the PRE publication */ 
    final double vLower = (1-epsilon)*vUpper+epsilon*(v-b*dt);
    final double r=Math.random(); // don't know the syntax; should be an instance of a uniform(0,1) distributed pseudorandom number
