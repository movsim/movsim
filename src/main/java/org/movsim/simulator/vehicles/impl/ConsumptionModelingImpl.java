...
public class ConsumptionModelingImpl {

    private static final String DEFAULT_DUMMY_LABEL="none";  // default from dtd
    
    /** The Constant logger. */
    final static Logger logger = LoggerFactory.getLogger(ConsumptionModelingImpl.class);

    private Map<String, FuelConsumption> fuelModelsMap;

    public ConsumptionModelingImpl(FuelConsumptionInput input) {
        if (input == null) {
            return;
        }
