...
public class OnrampImpl implements Onramp{
    ...
    public OnrampImpl(RampData rampData, VehicleGenerator vehGenerator, VehicleContainer mainVehContainer, String projectName, int rampIndex){
        ...
        if(rampData.withLogging()){
            ...
            fstrLogging.printf(Constants.COMMENT_CHAR+" time[s]  count[i]  xEnter[m]  vEnter[km/h]  laneEnter[1]  queue[1]  qBC[1/h]%n");
        }
    ...
    public void update(int itime, double dt, double time ) {
        ...
        // logger.debug("ramp update. nWait = {}, queue.size() = {}", nWait, vehicleQueue.size());
        ...
        if(vehicleQueue.size()>=1){
            ...
            if(isMerging){
                ...
                if(fstrLogging!=null){
                    fstrLogging.printf("%10.1f %6d  %8.2f  %8.2f  %5d  %5d  %8.2f%n", time, mergeCount, xEnterLastMerge, 3.6*vEnterLastMerge, laneEnterLastMerge, vehicleQueue.size(), 3600*qBC);
                    fstrLogging.flush();
                }
            }
        }
...
//    }
    ...
    private double speedToEnter(double speedLeader){
           ...
           double reducefact=1. - (1-RAMP_VEL_REDUCEFACTOR)* Math.min(1., speedLeader/vcrit);
    ...
    private boolean tryMerge(Vehicle vehToEnter, VehicleContainer mainVehContainer){
        ...
        logger.debug("");
        double xUp   = xCenter-0.5*length; //   = xCenter-0.5*length;
        double xDown = xCenter+0.5*length;;// = xCenter+0.5*length;
        //double vLead = Constants.MAX_VEHICLE_SPEED;;// = 200; // very large value //TODO
        
        final List<Vehicle> mainVehicles = mainVehContainer.getVehicles();
        //final int size = mainVehicles.size(); 
            
      //  ArrayList<TestMergeData> testMergeData = new ArrayList<TestMergeData>();
        ...
        if( mainVehSize == 0){
            ...
            final double vEnter = speedToEnter(vehToEnter.getDesiredSpeedParameter()); // no leader
            logger.debug("most downstream veh is still upstream of ramp center. mainVeh.size() = {}. posMostDown = {}", mainVehSize, mainVehDown.position());
            addVehicleFromRamp(vehToEnter, xEnter, vEnter, laneEnter);
            return true;
        }
            ...
            logger.debug("most upstream veh is already downstream of ramp center. mainVeh.size() = {}, posMostUp = {}", mainVehSize, mainVehUp.position());
            addVehicleFromRamp(vehToEnter, xEnter, vEnter, laneEnter);
            return true;
        }
        else{
            ...
            // logger.debug("mainVehContainer.getMostDownstream().position() = {} ", mainVehContainer.getMostDownstream().position());
            // logger.debug("mainVehContainer.getMostUpstream().position() = {} ", mainVehContainer.getMostUpstream().position());
            ...
            // scan through vehicle list on mainroad in upstream direction (increase index) 
            ...
            for(int i=indexDown; i<= Math.min(indexUp + 1, mainVehSize-1); i++){
                ...
                if(i == indexUp+1){
                    netGap = mainVehicles.get(i-1).position() - xEnterTest - 0.5*(mainVehicles.get(i-1).length()+vehToEnter.length());
                }
                ...
                if( netGap >  MINSPACE_MERGE_M && netGap > minGap ){
                    ...
                    minGap = netGap;
                    xEnter = xEnterTest;
                    vEnter = (i>0) ? speedToEnter( mainVehicles.get(i-1).speed()) : speedToEnter( vehToEnter.getDesiredSpeedParameter() );
                }
            }
            ...
            if(indexUp == mainVehSize){
                ...
                logger.debug("test gap for most upstream  without back veh: netGap = {}, xEnterTest = {}", netGap, xEnterTest );
                if( netGap >  MINSPACE_MERGE_M &&  netGap > minGap ){
                    //  new !!
                    minGap = netGap;
                    xEnter = xEnterTest;
...
//                    final double gap = actualVeh.netDistance(previousVeh); // single-lane simulation!
//                    if(gap < minGap){
//                        xEnter = ;
...
//        
//        public TestMergeData(double gap, double xCenter, double vVehDown, double vVehUp){
//            this.gap = gap;
