...
public class UpstreamBoundaryImpl implements UpstreamBoundary {
    ...
    public void update(int itime, double dt, double time) {
        ...
        if (nWait >= 1) {
            ...
            if (isEntered) {
                nWait--;
                if(fstrLogging!=null){
                    fstrLogging.printf("%10.1f %6d  %8.2f  %8.2f  %5d  %6.1f  %8.2f%n", time, enteringVehCounter, xEnterLast, 3.6*vEnterLast, laneEnterLast, nWait, 3600*qBC);
                    fstrLogging.flush();
                }
            }
        }
    ...
    private boolean tryEnteringNewVehicle(double time, double qBC) {
        ...
        final VehiclePrototype vehPrototype = vehGenerator.getVehiclePrototype(); // type of new vehicle
        ...
        final double netGapToLeader = leader.position() - leader.length(); // origin of road section is assumed to be zero
        ...
        final double minRequiredGap = 0.8 * gapAtQMax; // minimal distance set to 80 % of 1/rho at flow maximum in fundamental
                                                       // diagram
    ...
    private void enterVehicle(double time, double sFreeMin, VehiclePrototype vehPrototype, Vehicle leader) {
        ...
        double xLast = leader.position();
        double vLast = leader.speed();
        double aLast = leader.acc();

        double speedDefault = inflowTimeSeries.getSpeed(time);
        double vEnterTest = Math.min(speedDefault, 1.5 * vLast);
        double lengthLast = leader.length();
