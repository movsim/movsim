...
public class AdaptiveHistogramTest extends TestCase {
     ...
     * @param min the minimum value in the data set used to build the histogram.
     * @param max the maximum value in the data set used to build the histogram.
     * @param totalCount the total number of data points used to build the histogram.
     * @param skew true if the data is skewed, false otherwise.
     */
    private void checkValues (AdaptiveHistogram h, float min, float max, long totalCount, boolean skew) {
        ...
        assertTrue("AccumCount for a value higher than the maximum is not the whole data set", h.getAccumCount(max+alittle) == totalCount);
        if (!flat) assertTrue("AccumCount for the middle of the data set returns the whole data set", h.getAccumCount(middle) < totalCount);
        if (skew) assertTrue("AccumCount for the middle of a skewed data set doesn't reflect the skew", h.getAccumCount(middle) > 1.1f * (totalCount / 2));
        ...
        assertTrue("The value for percentile 0 is not close to the begining of the data set", h.getValueForPercentile(0) < min+alittle);
        assertTrue("The value for percentile 100 is not close to the end of the data set", h.getValueForPercentile(100) > max-alittle);
        if (skew) assertTrue("The value for percentile 50 of a skewed data set doesn't reflect the skew", h.getValueForPercentile(50) < middle-alittle);
     ...
     * @param data an array of numbers to pass on to the histogram.
     * @param min minimun value in the data set.
     * @param max maximun value in the data set.
     * @param skew true if the data is skewed, false otherwise.
     */
     ...
     * @param tests the number of data points.
     */
     ...
     * @param tests the number of data points.
     * @param val the fixed value to use in the test.
     */
