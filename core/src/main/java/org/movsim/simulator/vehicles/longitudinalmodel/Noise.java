...
/**
 * Acceleration noise for microscopic traffic models with (stochastic process) models for uncorrelated (white) noise
 * or temporally correlated values centered around zero (Ornstein-Uhlenbeck).
 *
 * <p>Paper for reference:
 * <p><a href="http://arxiv.org/abs/physics/0508222"> M. Treiber, A. Kesting, D. Helbing, Understanding widely scattered traffic flows, the
 * capacity drop, and platoons as effects of variance-driven time gaps. Phys. Rev. E 74, 016123 (2006).</a></p>
 */

// TODO formulate noise in more general terms. not only applicable to acceleration noise. Standard Ornstein-Uhlenbeck with fluctStrenth 1
public class Noise {
    ...
    /** Relaxation time in seconds (parameter) */
    private final double tauRelaxAcc;

    /** Fluctuation strength in (m^2/s^3) of dv/dt=a_det+xi(t). In case of Ornstein-Uhlenbeck process the standard deviation (parameter) */
    ...
    public Noise(NoiseParameter parameters) {
        ...
        isOrnsteinUhlenbeckProcess = tauRelaxAcc != 0;
        LOG.debug("tauRelaxAcc={}, isOrnsteinUhlenbeckProcess={}", tauRelaxAcc, isOrnsteinUhlenbeckProcess);
    }

    /**
     * Calculates the acceleration noise {code xiAcc} modelled by a Ornstein-Uhlenbeck stochastic process or as delta-correlated random process.
     *
     * @param dt simulation time interval in seconds
     */
    public void update(double dt) {
        ...
        if (isOrnsteinUhlenbeckProcess) {
            ...
            LOG.debug("Ornstein-Uhlenbeck process: betaAcc={}, stdDevAcc*Math.sqrt(2*dt/tauRelaxAcc)*randomMu0Sigma1= {}", betaAcc,
